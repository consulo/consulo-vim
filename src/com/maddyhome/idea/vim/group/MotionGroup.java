...
public class MotionGroup
{
	...
	private void processMouseClick(@NotNull Editor editor, @NotNull MouseEvent event)
	{
		...
		switch(CommandState.getInstance(editor).getSubMode())
		{
			case NONE:
				...
				int col = EditorHelper.normalizeVisualColumn(editor, vp.line, vp.column, CommandState.getInstance(editor).getMode() == CommandState.Mode.INSERT || CommandState.getInstance(editor)
						.getMode() == CommandState.Mode.REPLACE);
				if(col != vp.column)
				{
	...
	private void scrollColumnToScreenColumn(@NotNull Editor editor, int column)
	{
		...
		scrollColumnToLeftOfScreen(editor, EditorHelper.normalizeVisualColumn(editor, editor.getCaretModel().getVisualPosition().line, visualColumn - column + 1, false));
	}
	...
	private void scrollLineToScreenLine(@NotNull Editor editor, int line, int rawCount, int count, boolean start)
	{
		...
		int visualLine = rawCount == 0 ? editor.getCaretModel().getVisualPosition().line : EditorHelper.logicalLineToVisualLine(editor, count - 1);
		...
		if(visualLine != editor.getCaretModel().getVisualPosition().line || start)
		{
			...
			{
				offset = moveCaretVertical(editor, EditorHelper.visualLineToLogicalLine(editor, visualLine) - editor.getCaretModel().getLogicalPosition().line);
			}
	...
	public boolean scrollColumn(@NotNull Editor editor, int columns)
	{
		...
		visualColumn = EditorHelper.normalizeVisualColumn(editor, editor.getCaretModel().getVisualPosition().line, visualColumn + columns, false);
	...
	{
		return EditorHelper.normalizeOffset(editor, line, EditorHelper.getLineEndOffset(editor, line, allowPastEnd), allowPastEnd);
	}
	...
	{
		int oldOffset = editor.getCaretModel().getOffset();
		int offset = EditorHelper.normalizeOffset(editor, editor.getCaretModel().getLogicalPosition().line, oldOffset + count, allowPastEnd);
		if(offset == oldOffset)
		{
			return -1;
		}
	...
	{
		return moveCaretToLineStartSkipLeading(editor, rawCount == 0 ? EditorHelper.normalizeLine(editor, EditorHelper.getLineCount(editor) - 1) : line);
	}

	public int moveCaretGotoLineLastEnd(@NotNull Editor editor, int rawCount, int line, boolean pastEnd)
	{
		return moveCaretToLineEnd(editor, rawCount == 0 ? EditorHelper.normalizeLine(editor, EditorHelper.getLineCount(editor) - 1) : line, pastEnd);
	}
	...
	public static void scrollPositionIntoView(@NotNull Editor editor, @NotNull VisualPosition position, boolean scrollJump)
	{
	...
	public VisualChange getVisualOperatorRange(@NotNull Editor editor, int cmdFlags)
	{
		...
		int chars;
		SelectionType type;
		if(CommandState.getInstance(editor).getSubMode() == CommandState.SubMode.VISUAL_LINE || (cmdFlags & Command.FLAG_MOT_LINEWISE) != 0)
		{
			chars = ep.column;
	...
	{
		return new TextRange(editor.getSelectionModel().getBlockSelectionStarts(), editor.getSelectionModel().getBlockSelectionEnds());
	}
