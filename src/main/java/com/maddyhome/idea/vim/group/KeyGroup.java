...
public class KeyGroup {
    ...
    @NotNull
    private final Map<KeyStroke, ShortcutOwner> shortcutConflicts = new LinkedHashMap<KeyStroke, ShortcutOwner>();
    @NotNull
    private final Set<KeyStroke> requiredShortcutKeys = new HashSet<KeyStroke>();
    @NotNull
    private final HashMap<MappingMode, RootNode> keyRoots = new HashMap<MappingMode, RootNode>();
    @NotNull
    private final Map<MappingMode, KeyMapping> keyMappings = new HashMap<MappingMode, KeyMapping>();
    @Nullable
    ...
    public void registerRequiredShortcutKeys(@NotNull Editor editor) {
        ...
        EventFacade.getInstance().registerCustomShortcutSet(VimShortcutKeyAction.getInstance(), toShortcutSet(requiredKeys), editor.getComponent());
    }
    ...
    public void registerCommandAction(@NotNull VimCommandAction commandAction, @NotNull String actionId) {
        ...
        registerAction(commandAction.getMappingModes(), actionId, commandAction.getType(), commandAction.getFlags(), shortcuts.toArray(new Shortcut[shortcuts.size()]),
                commandAction.getArgumentType());
    }

    /**
     * @deprecated Inherit your action from {@link VimCommandAction} instead.
     */
    @Deprecated
     ...
     */
    @Deprecated
    public void registerAction(@NotNull Set<MappingMode> mappingModes, @NotNull String actName, @NotNull Command.Type cmdType, Shortcut shortcut, @NotNull Argument.Type argType) {
        //noinspection deprecation
     ...
     */
    @Deprecated
    public void registerAction(@NotNull Set<MappingMode> mappingModes, @NotNull String actName, @NotNull Command.Type cmdType, int cmdFlags, Shortcut shortcut, @NotNull Argument.Type argType) {
        //noinspection deprecation
     ...
     */
    @Deprecated
    public void registerAction(@NotNull Set<MappingMode> mappingModes, @NotNull String actName, @NotNull Command.Type cmdType, @NotNull Shortcut[] shortcuts, @NotNull Argument.Type argType) {
        //noinspection deprecation
     ...
     */
    @Deprecated
    public void registerAction(@NotNull Set<MappingMode> mappingModes,
    ...
    @Nullable
    private Node addNode(@NotNull ParentNode base, @NotNull String actName, @NotNull Command.Type cmdType, int cmdFlags, @NotNull KeyStroke key, @NotNull Argument.Type argType, boolean last) {
        // Lets get the actual action for the supplied action name
        ActionManager aMgr = ActionManager.getInstance();
        AnAction action = aMgr.getAction(actName);
        if (action == null) {
            LOGGER.warn("Action with id " + actName + " is not found");
            return null;
        }
    ...
    private static ShortcutSet toShortcutSet(@NotNull Collection<KeyStroke> keyStrokes) {
        final List<consulo.ui.ex.action.Shortcut> shortcuts = new ArrayList<consulo.ui.ex.action.Shortcut>();
        for (KeyStroke key : keyStrokes) {
            shortcuts.add(new KeyboardShortcut(key, null));
        }
        return new CustomShortcutSet(shortcuts.toArray(new consulo.ui.ex.action.Shortcut[shortcuts.size()]));
    }
        ...
        for (Component c = component; c != null; c = c.getParent()) {
            if (c instanceof JComponent) {
                ...
                for (AnAction action : actions) {
                    ...
                    final consulo.ui.ex.action.Shortcut[] shortcuts = action.getShortcutSet().getShortcuts();
                    for (consulo.ui.ex.action.Shortcut shortcut : shortcuts) {
